 ...
 */
 ...
 define("PIE_VALUE_AND_PERCENTAGE", 140022);
       ...
       foreach($Values as $Key => $Value)
        {
		 ...
		 $thisPadding = $ValuePadding;
	        ...
	        elseif ( $WriteValues == PIE_VALUE_AND_PERCENTAGE) {
		        $Display = $Value.$ValueSuffix.",\n".round(( 100 / $SerieSum ) * $Value,$Precision)."%";
		        $thisPadding = $ValuePadding;
	        }
         ...
         if ( $ValuePosition == PIE_VALUE_OUTSIDE )
          {
           $Xc = cos(($Angle-90)*PI/180) * ($Radius+$thisPadding) + $X;
           $Yc = sin(($Angle-90)*PI/180) * ($Radius+$thisPadding) + $Y;
          }
         else
          {
       ...
       foreach($Values as $Key => $Value)
        {
         ...
         elseif ( $WriteValues == PIE_VALUE_NATURAL )
          $Display = $Value.$ValueSuffix;
         elseif ( $WriteValues == PIE_VALUE_AND_PERCENTAGE)
	      $Display = $Value.$ValueSuffix.", ".round(( 100 / $SerieSum ) * $Value,$Precision)."%";
       ...
       foreach($Values as $Key => $Value)
        {
         ...
         elseif ( $WriteValues == PIE_VALUE_NATURAL )
          $Label = $Data["Series"][$Data["Abscissa"]]["Data"][$Key];
         elseif ( $WriteValues == PIE_VALUE_AND_PERCENTAGE)
	      $Label = $Data["Series"][$Data["Abscissa"]]["Data"][$Key].", ".round(( 100 / $SerieSum ) * $Value,$Precision)."%";
         else
          $Label = "";
